...
if __name__ == '__main__':
    ...
    for run_id in range(0, num_runs):
        for trim in trimming:
            ...
            fib_path = os.path.join(base_path, "fibs")
            ...
            for max_failures in all_max_failures:
                ...
                network, netenv, waypoints = build_network(ms_manager.backend, base_path, max_failures, waypoints_min, waypoints_fraction)
                ...
                logger.info("RunID {run_id} - Scenario {scenario} - Max Failures {failures}".format(
                            run_id=run_id, failures=max_failures, scenario=scenario))
                ...
                for sampling_mode in sampling_modes:
                    ...
                    dump_file = "../logs/pipeline_%s_%s_pdb_%s.csv" % (scenario, '{:%Y%m%d-%H%M%S}'.format(datetime.datetime.now()), sampling_mode)
                    policy_db.dump(dump_file)
