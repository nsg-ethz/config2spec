...
class PropertyAdder {
  ...
  private BoolExpr baseReachability(
      Context ctx,
      Graph g,
    ...
    return ctx.mkOr(hasDirectRoute, isAbsorbed);
  }
  ...
  Map<String, BoolExpr> instrumentReachability(Set<GraphEdge> ges) {
    ...
    for (Entry<String, List<GraphEdge>> entry : g.getEdgeMap().entrySet()) {
      ...
      // Add the base case, in which we consider all directly connected routers to be able
      // to reach the destination.
      BoolExpr guard = baseReachability(ctx, g, edges, ges, router);

      // Add the recursive case, where a router is able to reach the destination when it
      // forwards the traffic to a neighbor that can reach the destination
   ...
   * not reachable and we set the id to 0.
   */
  private BoolExpr recursiveNumPaths(
      Context ctx,
      EncoderSlice slice,
      List<GraphEdge> edges,
      Map<String, ArithExpr> idVars,
    ...
    ArithExpr one = ctx.mkInt(1);
    BoolExpr hasRecursiveRoute = ctx.mkFalse();
    ArithExpr numPaths = ctx.mkInt(0);
    for (GraphEdge edge : edges) {
      if (!edge.isAbstract()) {
        BoolExpr fwd = _encoderSlice.getForwardsAcross().get(router, edge);
        if (edge.getPeer() != null) {
          ...
          BoolExpr sendToReachable = ctx.mkAnd(fwd, peerReachable);
          hasRecursiveRoute = ctx.mkOr(hasRecursiveRoute, sendToReachable);
          numPaths = ctx.mkAdd(numPaths, (ArithExpr) ctx.mkITE(sendToReachable, one, zero));
        }
      }
    }
    return slice.mkIf(hasRecursiveRoute, ctx.mkEq(id, numPaths), ctx.mkEq(id, zero));
  }

  /*
   * Instruments the network with the number of available paths
   */
  Map<String, BoolExpr> instrumentNumPaths(Set<GraphEdge> ges, int k) {
    Context ctx = _encoderSlice.getCtx();
    Solver solver = _encoderSlice.getSolver();
    EncoderSlice slice = _encoderSlice;
    Map<String, ArithExpr> idVars = new HashMap<>();
    Map<String, BoolExpr> numPathsCheck = new HashMap<>();

    // Init all reachable variables that have a value larger or equal to zero and indicate that the
    // router can reach the destination if the value is larger than zero.
    String sliceName = slice.getSliceName();
    ArithExpr zero = ctx.mkInt(0);
    for (String r : _encoderSlice.getGraph().getRouters()) {
      int id = _encoderSlice.getEncoder().getId();
    ...
    for (Entry<String, List<GraphEdge>> entry : g.getEdgeMap().entrySet()) {
      ...
      // Add the recursive case, where it is reachable through a neighbor
      BoolExpr recursive = recursiveNumPaths(ctx, slice, edges, idVars, router, id);
      BoolExpr cond = slice.mkIf(guard, ctx.mkEq(id, ctx.mkInt(1)), recursive);
      solver.add(cond);

      ArithExpr kExpr = ctx.mkInt(k);
      BoolExpr hasKPaths = ctx.mkEq(id, kExpr);

      numPathsCheck.put(router, hasKPaths);
    }
    return numPathsCheck;
  }

  /*
   * Instruments the network with a sequence of waypoints
   */
  Map<String, BoolExpr> instrumentWaypointing(Set<GraphEdge> ges, List<String> waypoints) {
    Context ctx = _encoderSlice.getCtx();
    Solver solver = _encoderSlice.getSolver();

    Map<String, BoolExpr> reachableChecks = this.instrumentReachability(ges);

    // chainmatch_ROUTER_I is true if ROUTER has a path to the destination and the waypoint chain
    // has been matched up to the Ith element
    Map<String, BoolExpr> chainMatch = new HashMap<>();
    for (String router : _encoderSlice.getGraph().getRouters()) {
      for (int i = 0; i < waypoints.size(); i++) {
        String varName1 = "chainmatch_" + router + "_" + i;
        BoolExpr chainMatch1 = ctx.mkBoolConst(varName1);
        chainMatch.put(router + "_" + i, chainMatch1);
      }
    }
    ...
    for (Entry<String, List<GraphEdge>> entry : g.getEdgeMap().entrySet()) {
      ...
      int index = waypoints.indexOf(router);
      if (index >= 0) {
        // The router is part of the waypoint sequence - so, the state is updated to the next point

        // What has to hold
        BoolExpr match = ctx.mkTrue();
        BoolExpr notBlackhole = ctx.mkFalse();
        for (GraphEdge ge : edges) {
          if (!ge.isAbstract()) {
            BoolExpr fwd = _encoderSlice.getForwardsAcross().get(router, ge);

            if (ges.contains(ge) && index == 0) {
              notBlackhole = ctx.mkOr(notBlackhole, reachableChecks.get(router));
            }

            if (ge.getPeer() != null) {
              BoolExpr peerReaches = reachableChecks.get(ge.getPeer());
              BoolExpr condition = ctx.mkAnd(fwd, peerReaches);

              if (index == 0) {
                // Exception for the very first waypoint, make sure that the waypoint variables are
                // all set to False for all neighbors to which the router forwards traffic to
                BoolExpr prevMatch = ctx.mkTrue();
                for (int i = 0; i < waypoints.size(); i++) {
                  prevMatch = ctx.mkAnd(prevMatch, ctx.mkNot(chainMatch.get(ge.getPeer() + "_" + i)));
                }
                match = ctx.mkAnd(match, ((BoolExpr) ctx.mkITE(condition, prevMatch, ctx.mkTrue())));

              } else {
                // Check that for all neighbors to which this router forwards (in case of loadbalancing)
                // the same waypoints have been traversed.
                BoolExpr prevMatch = chainMatch.get(ge.getPeer() + "_" + (index-1));
                match = ctx.mkAnd(match, ((BoolExpr) ctx.mkITE(condition, prevMatch, ctx.mkTrue())));
              }

              notBlackhole = ctx.mkOr(notBlackhole, condition);
            }
          }
        }

        // Create the bi-implication part
        BoolExpr biImplication = ctx.mkTrue();
        for (int i = 0; i < waypoints.size(); i++) {
          BoolExpr thisMatch = chainMatch.get(router + "_" + i);
          if (i != index) {
            thisMatch = ctx.mkNot(thisMatch);
          }
          biImplication = ctx.mkAnd(biImplication, thisMatch);
        }

        // this is to exclude blackholes from consideration
        match = ctx.mkAnd(match, notBlackhole);

        solver.add(ctx.mkEq(match, biImplication));
      } else {
        // the router is not part of the waypoint sequence, so the state is just passed on

        for (int i = 0; i < waypoints.size(); i++) {
          BoolExpr biImplication = chainMatch.get(router + "_" + i);
          BoolExpr notBlackhole = ctx.mkFalse();
          ...
          for (GraphEdge ge : edges) {
            if (!ge.isAbstract() && ge.getPeer() != null) {
              ...
              BoolExpr peerReaches = reachableChecks.get(ge.getPeer());
              BoolExpr condition = ctx.mkAnd(fwd, peerReaches);
              BoolExpr prevMatch = chainMatch.get(ge.getPeer() + "_" + i);
              match = ctx.mkAnd(match, ((BoolExpr) ctx.mkITE(condition, prevMatch, ctx.mkTrue())));
              notBlackhole = ctx.mkOr(notBlackhole, condition);
            }
          }
          solver.add(ctx.mkEq(ctx.mkAnd(match, notBlackhole), biImplication));
        }
      }
    }

    // Prepare Expressions for Minesweeper to check the properties
    Map<String, BoolExpr> waypointChecks = new HashMap<>();
    for (String router : _encoderSlice.getGraph().getRouters()) {
      int lastIndex = waypoints.size() - 1;
      waypointChecks.put(router, chainMatch.get(router + "_" + lastIndex));
    }

    return waypointChecks;
  }
}